...
# V4.0 目录独立：QMem 全部资源（db/schema/embedding）与本文件同目录（mcp/qmem/）
_DIR = os.path.dirname(os.path.abspath(__file__))
DBPATH = os.path.join(_DIR, "core_memory.db")
LOG_DBPATH = os.path.join(_DIR, "call_log.db")

# V4.0 瘦身：QMem 只承载项目记忆（单层）。
# 领域知识图谱已独立为 DomainKG MCP，代码事实由 codebase-memory-mcp 独立提供。
# V4.1（2026-07-21）：彻底删除 tier / origin_project 列（原 V4.0 保留作迁移兼容，现已确认无代码读写）。
_REQUIRED_COLUMNS = {
    "title": "TEXT DEFAULT ''",
...
class QMemMCP:
    def __init__(self):
        ...
        # V4.0 瘦身：只剩 11 个项目记忆工具。consensus 工具族、kg 工具族、CBM 转发全部移除。
        # 代码事实查询（search_graph/trace_path 等）由 codebase-memory-mcp 独立 MCP server 提供。
        # 业务概念查询（concept_recall/concept_get 等）由 domain_mcp_server.py（DomainKG MCP）提供。
        self.local_tools = {
            "mem_save", "mem_recall", "mem_delete",
            "init_project_context", "mem_search", "mem_update", "mem_context",
            "mem_list_projects", "mem_get_full",
    ...
    def _tools_list(self):
        local_tools = [
            {"name": "mem_save", "description": "保存项目记忆（Push）。type 为必填（决定生命周期），topic_key 可选（仅记忆多时用于主题分类）。topic_key 命中时自动 upsert。★ 写入门禁：纯新增时若本项目已有相似度>0.85 的记忆，返回 candidates 拦截（防污染）；确认是新主题请传 force=true 放行，是同主题更新请改用 mem_update。★ 业务概念/技术规范不要存这里——业务概念查 DomainKG MCP 的 concept_recall，技术规范在全局 CLAUDE.md。", "inputSchema": {"type": "object", "properties": {"project_id": {"type": "string"}, "topic_key": {"type": "string", "description": "可选，主题分类（arch/workflow/m1.4 等）。记忆少时留空"}, "content": {"type": "string"}, "title": {"type": "string"}, "type": {"type": "string", "enum": ["reference", "progress", "decision", "bugfix", "learning", "manual"], "description": "必填。reference=稳定知识, progress=易过期进度(超30天需复审), decision=决策, bugfix=已修, learning=经验, manual=手动"}, "force": {"type": "boolean", "description": "默认 false。写入门禁拦截后，确认确为新主题时传 true 放行（绕过近邻检查）"}}, "required": ["project_id", "content", "type"]}},
            {"name": "mem_recall", "description": "RRF 混合检索：搜当前项目的动态记忆（FTS5+向量）。需求分析/业务概念查询请改用 DomainKG MCP 的 concept_recall。", "inputSchema": {"type": "object", "properties": {"query": {"type": "string"}, "current_project": {"type": "string"}, "min_similarity": {"type": "number", "default": 0.5}, "limit": {"type": "integer", "default": 10}}, "required": ["query"]}},
            {"name": "mem_search", "description": "精确/过滤查找：按 project/type 过滤项目记忆。", "inputSchema": {"type": "object", "properties": {"query": {"type": "string"}, "project": {"type": "string"}, "type": {"type": "string"}, "limit": {"type": "integer", "default": 20}}, "required": []}},
            ...
            {"name": "mem_context", "description": "开场召回：加载项目自身动态记忆。返回摘要索引（title + content前100字 + obs_id），不再返回全文。用 mem_get_full(obs_id=) 拉指定记忆的完整内容。★ 附带 review_queue：本项目超 30 天的 progress 记忆（可能过时），开场顺手用 mem_get_full 核实，已推进的 upsert 更新、已作废的软删。", "inputSchema": {"type": "object", "properties": {"project": {"type": "string"}}, "required": ["project"]}},
            ...
            {"name": "mem_delete", "description": "默认软删除记忆（标记 deleted_at，可恢复，防误删）。传 hard=true 走物理删除（清 FTS/向量索引）。", "inputSchema": {"type": "object", "properties": {"obs_id": {"type": "string"}, "hard": {"type": "boolean", "description": "默认 false 软删除；true 物理删除"}}, "required": ["obs_id"]}},
            {"name": "mem_list_projects", "description": "列出所有项目的记忆数。", "inputSchema": {"type": "object", "properties": {}}, },
            ...
            {"name": "cross_project_health_check", "description": "检测跨项目 q4 记忆的语义重复（embedding 相似度>阈值）。发现不同 project 间的高相似对时，提示 AI 判断是真重复还是同构模板，分别合并/精炼到各自项目。", "inputSchema": {"type": "object", "properties": {"threshold": {"type": "number", "default": 0.85, "description": "相似度阈值，默认0.85"}, "limit": {"type": "integer", "default": 20, "description": "返回的高相似对数量上限"}}, "required": []}},
            {"name": "mem_consolidate_project", "description": "★ 单项目内高相似簇检测：找出同一 project 内互相相似的 q4 记忆对（embedding>阈值），提示 AI 合并精炼以消化单项目堆积。检测到后用 mem_update 融合内容，再 mem_delete 软删冗余条目。", "inputSchema": {"type": "object", "properties": {"project": {"type": "string", "description": "要检测的单个 project 名"}, "threshold": {"type": "number", "default": 0.85, "description": "相似度阈值，默认0.85"}, "limit": {"type": "integer", "default": 30, "description": "返回的高相似对数量上限"}}, "required": ["project"]}},
        ]
    ...
    def _dispatch_tool(self, name, args, params):
        """实际工具分发逻辑。V4.0：只分发 11 个项目记忆工具，不再转发 CBM。"""
        if name not in self.local_tools:
            raise ValueError(f"Local tool not found: {name}（业务概念查 DomainKG MCP，代码事实查 codebase-memory MCP）")
    ...
    def _save(self, args):
        ...
        # upsert：同 project+topic_key 存在则更新
        existing = None
        if topic_key:
        ...
        if not force:
            dup = self._nearest_neighbor(conn, vec, project_id, threshold=0.85, limit=3)
            if dup:
                conn.close()
                cands = [
    ...
    def _nearest_neighbor(self, conn, query_vec, project, threshold=0.85, limit=3):
        """写入门禁/去重共用的近邻查询：在指定 project 范围内找 query_vec 的相似记忆。
        返回相似度 > threshold 的存活记忆列表（含 obs_uuid/title/type/topic_key/sim），按相似度降序，最多 limit 条。
        用 vec_distance_cosine 函数式 API（与 search_rrf.semantic_search 同款），先按 project 过滤再算距离排序。
        ...
        try:
            ...
            rows = conn.execute(
                ...
                "WHERE mf.project=? AND mf.deleted_at IS NULL "
                "ORDER BY distance LIMIT ?",
                (v, project, max(limit * 3, 10))
            ).fetchall()
        except Exception as e:
    ...
    def _delete(self, args):
        ...
        conn = self._get_conn()
        row = conn.execute("SELECT 1 FROM memory_facts WHERE obs_uuid=?", (obs_id,)).fetchone()
        if not row:
            conn.close()
            return {"error": "not found"}
    ...
    def _recall(self, args):
        ...
        # V4.0 单层检索：只查当前项目的记忆，无共识域、无配额
        search_projects = [project] if project else None
        ranked = self.searcher.hybrid_search_rrf(
            query, vec,
            projects=search_projects,
            min_similarity=min_sim, limit=total_limit
        )

        # ★ 去重 + 质量信号：堵检索出口污染
        ranked = self._enrich_results(ranked, dedup_threshold=0.9)
        return {"results": ranked, "query": query, "count": len(ranked)}
        ...
        try:
            ...
            for r in enriched:
                ...
                if not tk:
                    ...
                    continue
                sup = conn2.execute(
                    "SELECT 1 FROM memory_facts WHERE project=? AND topic_key=? "
    ...
    def _search(self, args):
        ...
        conn = self._get_conn()
        if not query:
            sql = ("SELECT obs_uuid, project, topic_key, title, type, created_at "
                   "FROM memory_facts WHERE deleted_at IS NULL")
            params = []
        ...
        safe = query.replace('"', '""')
        sql = ("SELECT mf.obs_uuid, mf.project, mf.topic_key, mf.title, mf.content, mf.type, mf.created_at "
        ...
        try:
            ...
            p = f"%{query}%"
            sql = ("SELECT obs_uuid, project, topic_key, title, content, type, created_at "
    ...
    def _context(self, args):
        ...
        # 2. ★ 过期复审队列：本项目 type=progress 且超 PROGRESS_REVIEW_DAYS 天未更新的存活记忆
        review_queue = conn.execute(
            "SELECT obs_uuid, title, topic_key, type, created_at, updated_at "
            "FROM memory_facts "
            "WHERE deleted_at IS NULL AND project=? AND type='progress' "
    ...
    def _get_full(self, args):
        ...
        rows = conn.execute(
            f"SELECT obs_uuid, project, topic_key, title, content, type, "
    ...
    def _cross_project_health_check(self, args):
        """检测跨项目记忆的语义重复（embedding 相似度 > 阈值）。
        V4.0：QMem 只剩项目记忆，发现不同 project 间高相似对时提示 AI 判断真重复 vs 同构模板。"""
        ...
        for sim, ma, mb in pairs:
            results.append({
                ...
                "suggestion": (
                    f"跨项目语义重复。判断：① 是同一知识 → 较完善一方保留，另一方用 mem_delete 软删；"
                    f"② 是同构模板（如多地区相同业务）→ 各自保留，无需合并。"
                    f"业务概念（非项目工程记忆）应入 DomainKG MCP，不存 QMem。"
                )
            })
    ...
    def _consolidate_project(self, args):
        """★ 单项目内高相似簇检测：找出同一 project 内互相相似的记忆对，
        提示 AI 合并精炼以消化单项目堆积。
        ...
        rows = conn.execute(
            ...
            "FROM memory_facts mf JOIN memory_vectors mv ON mv.rowid = mf.id "
            "WHERE mf.deleted_at IS NULL AND mf.project = ?",
            (project,)
        ).fetchall()
        conn.close()
